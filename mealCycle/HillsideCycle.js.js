//Hillside

const cycle1 = {
    Monday: {
        Breakfast: ['Pancakes M/W/S', 'Bacon', 'Cottage Fries'],
        Lunch: [
            'Ravioli Italian Soup E/M/W',
            'Chicken Noodle Soup W',
            'Buffalo Chicken Salad M/W/S',
            'Spicy Miso Pork Ramen E/F/W/S',
            'Roasted Harissa Chicken',
            'Stuffed Bell Peppers',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Kung Pao Chicken S (Hillside only)',
        ],
        Dinner: [
            'Ravioli Italian Soup E/M/W',
            'Chicken Noodle Soup W',
            'Buffalo Chicken Salad M/W/S',
            'Chicken Tenders W',
            'Texas Style Pork Ribs',
            'Kung Pao Gardein Chicken S/W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Kung Pao Chicken S (Hillside only)',
        ],
    },
    Tuesday: {
        Breakfast: ['Biscuit and Country Gravy M/W/S', 'Kielbasa S', 'Hash Browns'],
        Lunch: [
            'Minestrone Soup W',
            'Potato and Bacon Soup M/W/S',
            'Kale and Quinoa Salad',
            'Grilled Thai Chicken',
            'Burrito Bowl',
            'Pasta w/ Gardein Meat Sauce S/W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Beef and Vegetable Stir Fry S/W (Hillside only)',
        ],
        Dinner: [
            'Minestrone Soup W',
            'Potato and Bacon Soup M/W/S',
            'Kale and Quinoa Salad',
            'Meat Lasagna W/E/M/S',
            'Chicken w/Lemon Herb Sauce',
            'Brown Rice and Quinoa w/Roasted Tomato',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Beef and Vegetable Stir Fry S/W (Hillside only)',
        ],
    },
    Wednesday: {
        Breakfast: ['Chorizo and Egg Burrito E/W/S', 'Turkey Sausage Links', 'Country Potatoes'],
        Lunch: [
            'Corn Chowder Soup M/S/W',
            'Posole  Soup',
            'Greek Salad M',
            'Baked Ziti W/M',
            'Grilled Beef Flank Steak w/Chimichurri Sauce',
            'Gardein Orange Chicken S/W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Teriyaki Chicken Stir Fry S/W (Hillside only)',
        ],
        Dinner: [
            'Corn Chowder Soup M/S/W',
            'Posole  Soup',
            'Greek Salad M',
            'BBQ Chicken Sandwich W',
            'Pork Chile Verde',
            'Gardein Chicken Tenders S/W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Teriyaki Chicken Stir Fry S/W (Hillside only)',
        ],
    },
    Thursday: {
        Breakfast: ['Sausage, Egg and Cheese English Muffin Sandwich E/M/W', 'Sausage Patty', 'Tater Tots'],
        Lunch: [
            'Chicken and Rice Soup',
            'Tomato Basil Soup M/W',
            'RYO California Sushi F/E/W/S/SF-C',
            'Pasta and Meatballs M/W/S/E',
            'Hamburger Bar M/W',
            'Gardein Beef w/Black Pepper Sauce S/W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Szechuan Crispy Beef Stir Fry S/W/SF-C (Hillside only)',
        ],
        Dinner: [
            'Chicken and Rice Soup',
            'Tomato Basil Soup M/W',
            'RYO California Sushi F/E/W/S/SF-C',
            'Teriyaki Chicken W/S',
            'Loaded Grilled Cheese Sandwich M/W/S',
            'Vegetable Lentil Stew',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only) ',
            'Szechuan Crispy Beef Stir Fry S/W/SF-C (Hillside only)',
        ],
    },
    Friday: {
        Breakfast: ['French Toast E/M/W', 'Bacon', "Potatoes O'Brien"],
        Lunch: [
            'Clam Chowder M/W/SF-C',
            'Vegetable Noodle Soup W',
            'Apple Walnut Salad TN',
            'Lemon Pepper Salmon F',
            'Moroccan Chicken w/Vegetables',
            'Edamame Fried Rice S/W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Edamame Fried Rice S/W(Hillside only)',
        ],
        Dinner: [
            'Clam Chowder M/W/SF-C',
            'Vegetable Noodle Soup W',
            'Apple Walnut Salad TN',
            'Beef Red Chile Colorado',
            'Fish and Chips F/W/M',
            'Gardein Chicken and Broccoli W/S',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Edamame Fried Rice S/W(Hillside only)',
        ],
    },
    Saturday: {
        Brunch: [
            'Croissant Sandwich W/E/M/S',
            'Kielbasa S',
            'Cottage Fries',
            'Italian Wedding  Soup W/S/M',
            'Broccoli Cheddar Soup M/W/S',
            "Chef's Choice",
            'Build Your Own Crepe',
        ],
        Dinner: [
            'Italian Wedding  Soup W/S/M        ',
            'Broccoli Cheddar Soup M/W/S        ',
            'Beef Enchiladas S/W/M        ',
            'Gardein FajitasS/W',
            'Pasta Bar W/M(Hillside only)            ',
            'Fish and Chips F/W (Beachside only)            ',
        ],
    },
    Sunday: {
        Brunch: [
            'Waffle Sticks E/M/S/W        ',
            'Sausage Link        ',
            'Hash Brown Patty        ',
            'Beef Barley Soup W        ',
            'Potato Chowder Soup W/M/S            ',
            "Chef's Choice            ",
        ],
        Dinner: [
            'Beef Barley Soup W        ',
            'Potato Chowder Soup W/M/S        ',
            'Chicken Parmesan M/W/S        ',
            'Baked Tortellini w/Spinach E/M/W            ',
            'Szechuan Tofu and Eggplant S/W            ',
        ],
    },
};

const cycle2 = {
    Monday: {
        Breakfast: ['Pancakes M/W/S        ', 'Turkey Sausage        ', 'Tater Tots        '],
        Lunch: [
            'Chicken Gumbo Soup  E/M/S/W/F        ',
            'Butternut Squash Soup  M        ',
            'Pesto Tortellini Salad  M/W/E        ',
            'Breaded Chicken Creamy Chipotle Pasta W/M (Hillside and Parkside only)            ',
            'Beef Tri Tip w/ Caramelized Onions            ',
            'The Buddha Bowl            ',
            "Chef's Choice             ",
            'Build Your Own Grain Bowl (Hillside only)            ',
            'Black Pepper Beef Stir Fry S/W (Hillside only)            ',
        ],
        Dinner: [
            'Chicken Gumbo Soup E/M/S/W/F        ',
            'Butternut Squash Soup M        ',
            'Pesto Tortellini Salad M/W/E        ',
            'Beef Taquitos S/W (Hillside and Parkside only)        ',
            'Butter Chicken M            ',
            'Creamy Cauliflower Mac and Cheese W            ',
            "Chef's Choice             ",
            'Build Your Own Grain Bowl (Hillside only)            ',
            'Black Pepper Beef Stir Fry S/W (Hillside only)            ',
        ],
    },
    Tuesday: {
        Breakfast: ['Chilaquiles M        ', 'Bacon', 'Hash Browns        '],
        Lunch: [
            'Broccoli Cheddar Soup M/W/S        ',
            'Chicken Tortilla Soup S/W        ',
            'Cobb Salad M/E        ',
            'Filipino Pork S/W (Hillside and Parkside only)            ',
            'Beef Hot Dog Bar W/M (Hillside and Parkside only)            ',
            'Vegetable Tangine            ',
            "Chef's Choice             ",
            'Build Your Own Grain Bowl (Hillside only)            ',
            'Mongolian Stir Fry Chicken S/W(Hillside only)            ',
        ],
        Dinner: [
            'Broccoli Cheddar Soup M/W/S        ',
            'Chicken Tortilla Soup S/W        ',
            'Cobb Salad M/E        ',
            'Chicken and Black Pepper Sauce S/W            ',
            'Nacho Bar M(Beachside and Hillside only)            ',
            'Fried Avocado Tacos W/M/S/E            ',
            "Chef's Choice             ",
            'Build Your Own Grain Bowl (Hillside only)            ',
            'Mongolian Stir Fry Chicken S/W(Hillside only)            ',
        ],
    },
    Wednesday: {
        Breakfast: ['Breakfast Burrito E/W/M        ', 'Pork Links        ', 'Country Potatoes        '],
        Lunch: [
            'Tomato Basil Soup M/W        ',
            'Chicken Noodle Soup W        ',
            'RYO Spicy California Sushi F/E/W/S/SF-C        ',
            'Orange Chicken Bowl W/S            ',
            'Pasta Beef Bolognese W (Hillside and Parkside only)            ',
            'Tofu w/White Bean Stew S            ',
            "Chef's Choice             ",
            'Build Your Own Grain Bowl (Hillside only)            ',
            'Sweet and Sour Chicken Stir Fry W/S (Hillside only)            ',
        ],
        Dinner: [
            'Tomato Basil Soup M/W        ',
            'Chicken Noodle Soup W        ',
            'RYO Spicy California Sushi F/E/W/S/SF-C        ',
            'Cajun Fish F            ',
            'Beef Bulgogi Fries W/S/M            ',
            'Vegetable Barley Stew W            ',
            "Chef's Choice             ",
            'Build Your Own Grain Bowl (Hillside only)            ',
            'Sweet and Sour Chicken Stir Fry W/S (Hillside only)            ',
        ],
    },
    Thursday: {
        Breakfast: ['Biscuit and Country Gravy M/W/S        ', 'Turkey Bacon        ', 'Cottage Fries        '],
        Lunch: [
            'Creamy Cauliflower Soup         ',
            'Caldo de Res Soup        ',
            'Brown Rice and Quinoa Kale Salad        ',
            'BBQ Pulled Pork Sandwich W(Hillside and Parkside only)            ',
            'Peruvian Roasted Chicken            ',
            'Gardein Chicken Stir Fry S/W            ',
            "Chef's Choice             ",
            'Build Your Own Grain Bowl (Hillside only)            ',
            'Char Sui Pork Stir Fry S/W(Hillside only)            ',
        ],
        Dinner: [
            'Creamy Cauliflower Soup        ',
            'Caldo de Res Soup        ',
            'Brown Rice and Quinoa Kale Salad        ',
            'Sweet and Spicy Pork S/W(Beachside only) Mac and Cheese M/W/S(Hillside and Parkside only)            ',
            'Chipotle Chicken Bowl (Hillside and Parkside only)            ',
            'Gardein Beef and Onions S/W            ',
            "Chef's Choice             ",
            'Build Your Own Grain Bowl (Hillside only)            ',
            'Char Sui Pork Stir Fry S/W(Hillside only)            ',
        ],
    },
    Friday: {
        Breakfast: ['Egg, Ham and Cheese on Croissant Sandwich E/W/M/S        ', 'Lil Smokies        ', "Potatoes O'Brien        "],
        Lunch: [
            'Minestrone Soup W         ',
            'Clam Chowder M/W/SF-C        ',
            'Elote in a Cup E/M        ',
            'Fish Tacos F/E (Hillside and Parkside only)            ',
            'Chicken Shawarma Bowl M            ',
            'Thai Curry Tofu S            ',
            "Chef's Choice             ",
            'Build Your Own Grain Bowl (Hillside only)            ',
            'Kung Pao Tofu S/W(Hillside only)            ',
        ],
        Dinner: [
            'Minestrone Soup W         ',
            'Clam Chowder M/W/SF-C        ',
            'Elote in a Cup E/M        ',
            'Wings and Things W            ',
            'Salmon Florentine F/M/W(Hillside and Parkside only)            ',
            'Asian Noodles w/Vegetables W/S            ',
            "Chef's Choice             ",
            'Build Your Own Grain Bowl (Hillside only)            ',
            'Kung Pao Tofu S/W (Hillside only)            ',
        ],
    },
    Saturday: {
        Brunch: [
            'Breakfast Sandwich on Naan Bread E/W/M/S        ',
            'Bacon',
            'Home Style Potatoes         ',
            'Split Pea Soup        ',
            'Miso Soup F/S            ',
            "Chef's  Choice            ",
            'Build Your Own Crepe (Hillside only)            ',
        ],
        Dinner: [
            'Split Pea Soup        ',
            'Miso Soup F/S        ',
            'Chicken Barbacoa Taco (Hillside only)        ',
            'Shrimp Pesto Pasta SF-C/W/M            ',
            'Bean and Cheese Pupusa M            ',
        ],
    },
    Sunday: {
        Brunch: [
            'Chocolate Chip Pancakes M/W/S        ',
            'Sausage Patty        ',
            'Hash Brown Patty        ',
            'Beef and Rice Soup             ',
            'Vegetable Noodle Soup W            ',
            "Chef's Choice             ",
            'Omelette Bar E/M            ',
        ],
        Dinner: ['Beef and Rice Soup         ', 'Vegetable Noodle Soup W        ', 'Ratatouille'],
    },
};

const cycle3 = {
    Monday: {
        Breakfast: ['French Toast E/M/W        ', 'Kielbasa S        ', 'Hash Browns        '],
        Lunch: [
            'Broccoli Cheddar Soup M/W/S        ',
            'Chicken and Rice Soup        ',
            'Citrus Kale Salad        ',
            'Hamburger Bar W/M (Hillside and Parkside only)            ',
            'Cheese Enchilada Suizas M/W            ',
            'Pasta alla Norma W            ',
            "Chef's Choice             ",
            'Orange Crispy Beef Stir Fry S/W/SE-C (Hillside only)            ',
        ],
        Dinner: [
            'Broccoli Cheddar Soup M/W/S        ',
            'Chicken and Rice Soup                                   ',
            'Citrus Kale Salad            ',
            'Beijing Beef S/W/SF-C            ',
            'Burrito Bowl',
            'Kung Pao Tofu S',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Orange Crispy Beef Stir Fry S/Ws/SE-C (Hillside only)',
        ],
    },
    Tuesday: {
        Breakfast: ['Sausage, Egg and Cheese English Muffin Sandwich W/E/M/S', 'Bacon', 'Cottage Fries'],
        Lunch: [
            'Italian Wedding Soup S/W/M',
            'Cream of Asparagus Soup M/W/S',
            'Cauliflower Ceviche',
            'Mongolian Chicken Drumstick W/S',
            'Beef Fajitas (Hillside and Parkside only)',
            'Gardein Meatball Sub W/S',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Lemon Chicken Stir Fry S/W (Hillside only)',
        ],
        Dinner: [
            'Italian Wedding Soup S/W/M',
            'Cream of Asparagus Soup M/W/S',
            'Cauliflower Ceviche',
            'Pasta Alfredo w/Broccoli W/M (Hillside and Parkside only)',
            'Popcorn Chicken Bowl W/M',
            'Gardein Beef Teriyaki Bowl S/W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Lemon Chicken Stir Fry S/W (Hillside only)',
        ],
    },
    Wednesday: {
        Breakfast: ['Hawaiian Breakfast E', 'SPAM/Ham', 'Country Potatoes/Rice'],
        Lunch: [
            'Tomato Basil Soup M/W',
            'Vegetable Noodle Soup W',
            'RYO Veggie Sushi',
            'Meatloaf W/E (Hillside and Parkside only)',
            'Tandoori Chicken W/M/S',
            'Potato and Cauliflower Curry',
            "Chef's Choice",
            'Teriyaki Pork Stir Fry S/W (Hillside only)',
        ],
        Dinner: [
            'Tomato Basil Soup M/W',
            'Vegetable Noodle Soup W',
            'RYO Veggie Sushi',
            'Lentil Tacos Citrus Glazed Chicken W/M/S (Hillside and Parkside only)',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Teriyaki Pork Stir Fry S/W (Hillside only)',
        ],
    },
    Thursday: {
        Breakfast: ['Blueberry Pancakes M/W/S', 'Pork Links', 'Tater Tots'],
        Lunch: [
            'Potato Chowder Soup W/M/S',
            'Posole Soup',
            'Chicken Taco Salad M/W/E',
            'Cheese Ravioli W/E/M (Hillside and Parkside only)',
            'BBQ Pork Ribs',
            'Lentil Chili Mac W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Kung Pao Chicken S (Hillside only)',
        ],
        Dinner: [
            'Potato Chowder Soup W/M/S',
            'Posole Soup',
            'Chicken Taco Salad M/W/E',
            'Turkey Pesto Flatbread M/W/E',
            'Cowboy Beef Flank Steak',
            'Tofu Fried Rice S/W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Kung Pao Chicken S (Hillside only)',
        ],
    },
    Friday: {
        Breakfast: ['Breakfast Burrito E/W/M', 'Turkey Sausage', "Potatoes O'Brien"],
        Lunch: [
            'Minestrone  Soup W',
            'Clam Chowder M/W/SF-C',
            'Arugula Salad M',
            'Street Beef Hot Dog W (Hillside and Parkside only)',
            'Firecracker Salmon F/S/W',
            'Szechuan Tofu and Veggies S/W',
            "Chef's Choice ",
            'Build Your Own Grain Bowl (Hillside only)',
            'Spicy Shrimp Stir Fry S/W/SF-C (Hillside only)',
        ],
        Dinner: [
            'Minestrone Soup W',
            'Clam Chowder M/W/SF-C',
            'Arugula Salad M ',
            'Lemon Chicken W/S (Hillside and Parkside only)',
            'Shrimp Scampi SF-C/W/M',
            'Pasta w/Gardein Meat Sauce S/W',
            "Chef's Choice ",
            'Build Your Own Grain Bowl (Hillside only)',
            'Spicy Shrimp Stir Fry S/W/SF-C (Hillside only)',
        ],
    },
    Saturday: {
        Brunch: [
            'Waffle Sticks E/M/S/W',
            'Bacon',
            'Home Fries ',
            'Chicken Gumbo Soup E/M/S/W/F',
            'Miso Soup F/S',
            "Chef's Choice",
            'Build Your Own Crepe (Hillside only)',
        ],
        Dinner: [
            'Chicken Gumbo Soup E/M/S/W/F',
            'Miso Soup F/S',
            'Steak Quesadilla M/W',
            'Buffalo Chicken Fries W',
            'Chana Masala Chickpeas',
        ],
    },
    Sunday: {
        Brunch: [
            'French Toast Sticks E/M/W/S',
            'Sausage Links',
            'Hash Brown Patty',
            'Potato Leek Soup M/S/W',
            'Albondigas Soup',
            "Chef's Choice",
        ],
        Dinner: [
            'Potato Leek Soup M/S/W',
            'Albondigas Soup',
            'Irish Nacho Bar M',
            'Chicken Bacon Alfredo Pasta W/M',
            'Gardein Beef Fajitas S/W',
        ],
    },
};

const cycle4 = {
    Monday: {
        Breakfast: ['Chocolate Chip Pancakes M/W/S', 'Sausage Patty', 'Tater Tots'],
        Lunch: [
            'Vegetarian Chili W',
            'Chicken Noodle Soup W',
            'Mandarin Spinach Salad W/S',
            'Pesto  Alfredo Tortellini M/W/E',
            'Chicken Fajitas (Hillside and Parkside Only)',
            'Vegan Cauliflower Tacos',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Kung Pao Chicken S (Hillside only)',
        ],
        Dinner: [
            'Vegetarian Chili W',
            'Chicken Noodle Soup W',
            'Mandarin Spinach Salad W/S',
            'Carnita Bowl (Hillside and Parkside only)',
            'Pasta w/Turkey Meat Sauce W',
            'Thai Chickpea Curry',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Kung Pao Chicken S (Hillside only)',
        ],
    },
    Tuesday: {
        Breakfast: ['Chilaquiles M', 'Pork Links', 'Hash Browns'],
        Lunch: [
            'Cauliflower Soup',
            'Chicken Tortilla Soup W/S',
            'BBQ Chicken Salad M/E',
            'Chicken Shawarma W/M/E (Hillside only)',
            'Pork Udon W/F/S/E (Hillside and Parkside only)',
            'Edamame Fried Rice S/W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Beef and Vegetables Stir Fry S/W (Hillside only)',
        ],
        Dinner: [
            'Cauliflower Soup',
            'Chicken Tortilla Soup W/S',
            'BBQ Chicken Salad M/E',
            'Kimchi Fried Rice w/Beef S/W/F',
            'Italian Sausage Sub W',
            'Sweet Potato Patties E/M/W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Beef and Vegetables Stir Fry S/W (Hillside only)',
        ],
    },
    Wednesday: {
        Breakfast: ['French Toast E/M/W', 'Turkey Sausage', 'Country Potatoes'],
        Lunch: [
            'Broccoli Cheddar M/W/S',
            'Menudo Soup',
            'Lemon Basil Pasta Salad M/W',
            'Beef Sliders W (Hillside and Parkside only)',
            'Japanese Curry Bowl W/S/M (Hillside and Parkside only)',
            'Vegetable Strudel W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Teriyaki Chicken Stir Fry S/w (Hillside only)',
        ],
        Dinner: [
            'Broccoli Cheddar M/W/S',
            'Menudo Soup',
            'Lemon Basil Pasta Salad M/W',
            'Teriyaki Chicken S/W',
            'Crispy Fish Po Boy Sandwich M/F/W/E',
            'Teriyaki Tofu S/W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Teriyaki Chicken Stir Fry S/W (Hillside only)',
        ],
    },
    Thursday: {
        Breakfast: ['Breakfast Sandwich on Naan Bread E/W/M/S', 'Lil Smokies', 'Tater Tots'],
        Lunch: [
            'Vegetable Noodle Soup W',
            'Tomato Basil Soup M/W',
            'Israeli Couscous on Arugula W',
            'Pasta Arrabiata W (Hillside and Parkside only)',
            'Beef Flank Steak w/Chimichurri Sauce',
            'Gardein Beef w/Black Pepper Sauce S/W (Hillside and Parkside only)',
            "Chef's Choice",
            'Cheeseburger Pizza S/W/M (Beachside and Hillside only)',
            'Build Your Own Grain Bowl (Hillside only)',
            'Szechuan Crispy Beef Stir Fry S/W/SF-C (Hillside only)',
        ],
        Dinner: [
            'Vegetable Noodle Soup W',
            'Tomato Basil Soup M/W',
            'Israeli Couscous on Arugula W',
            'Taco Bar (Hillside and Parkside only)',
            'Herb Baked Chicken',
            'Gardein Beef w/Black Pepper Sauce S/W (Hillside and Parkside only)',
            "Chef's Choice",
            'Cheeseburger Pizza S/W/M (Beachside and Hillside only)',
            'Build Your Own Grain Bowl (Hillside only)',
            'Szechuan Crispy Beef Stir Fry S/W/SF-C (Hillside only)',
        ],
    },
    Friday: {
        Breakfast: ['Ham, Egg and Cheese English Muffin Sandwich E/M/W/S', 'Bacon', 'Home Style Potatoes'],
        Lunch: [
            'Clam Chowder M/W/SF-C',
            'Ravioli Italian Soup E/M/W',
            'Fried Chicken Salad M/W',
            'Yucatan Chicken Grain Bowl (Hillside and Parkside only)',
            'Shrimp Alfredo M/W/SF-C',
            'Vegetable Kale Stew',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Lemon Shrimp Stir Fry SF-C/S/W (Hillside only)',
        ],
        Dinner: [
            'Clam Chowder M/W/SF-C',
            'Ravioli Italian Soup E/M/W',
            'Fried Chicken Salad M/W',
            'Orange Chicken W/S (Hillside and Parkside only)',
            'Lemon Pepper Tilapia F',
            'Gardein Orange Chicken W/S',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Lemon Shrimp Stir Fry SF-C/S/W (Hillside only)',
        ],
    },
    Saturday: {
        Brunch: [
            'Pancakes M/W/S',
            'Turkey Bacon ',
            "Potatoes O'Brien",
            'Vegetable Rice Soup (Hillside only)',
            'Beef and Barley Soup W (Hillside only)',
            "Chef's Choice",
            'Build Your Own Crepe (Hillside only)',
        ],
        Dinner: [
            'Vegetable Rice Soup (Hillside only)',
            'Beef and Barley Soup W (Hillside only)',
            'Irish Nacho Bar M (Hillside only)',
            'Spicy Ranch Chicken Sandwich W/S/E/M (Hillside only)',
            'Vegetable and Tofu Fried Rice S/W',
        ],
    },
    Sunday: {
        Brunch: [
            'French Toast E/M/W',
            'Sausage Link',
            'Hash Brown Patty',
            'Vegetable Rice Soup',
            'Beef and Barley Soup W',
            "Chef's Choice",
        ],
        Dinner: [
            'Vegetable Rice Soup',
            'Beef and Barley Soup W',
            'Chicken Birria Torta W',
            'Memphis Pork Ribs',
            'Pasta w/Gardein Meat Sauce W/S',
        ],
    },
};

const cycle5 = {
    Monday: {
        Breakfast: ['Conffeti Pancakes M/W/S', 'Kielbasa S', 'Cottage Fries'],
        Lunch: [
            'Posole Soup',
            'Broccoli Cheddar Soup M/W/S',
            'Chicken Taco Salad M/W/E',
            'Creamy Tomato Cheese Ravioli M/W/E',
            'Chicken Tenders W (Hillside and Parkside only)',
            'Soyrizo Jambalaya S',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Black Pepper Beef Stir Fry (Hillside only)',
        ],
        Dinner: [
            'Posole Soup',
            'Broccoli Cheddar Soup M/W/S',
            'Chicken Taco Salad M/W/E',
            'Herb Roasted Turkey',
            'Hamburger Bar W/M (Hillside and Parkside only)',
            'Tahini Chickpea Bowl W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Black Pepper Beef Stir Fry (Hillside only)',
        ],
    },
    Tuesday: {
        Breakfast: ['French Toast E/M/W', 'Bacon', 'Hash Browns'],
        Lunch: [
            'Miso Soup F/S',
            'Italian Wedding Soup S/W/M',
            'Kale Salad M',
            'Kahlua Pork Bowl',
            'Thai Chicken Drunken Noodles W/S (Hillside and Parkside only)',
            'Gardein Chicken Tenders S/W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Mongolian Stir Fry Chicken S/W (Hillside only)',
        ],
        Dinner: [
            'Miso Soup F/S',
            'Italian Wedding Soup S/W/M',
            'Kale Salad M',
            'Pasta w/Alfredo Sauce W/M (Hillside and Parkside only)',
            'Beef and Broccoli S/W',
            'Gardein Enchilada Scaloppine S/W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Mongolian Stir Fry Chicken S/W (Hillside only)',
        ],
    },
    Wednesday: {
        Breakfast: ['Breakfast Burrito E/W/M', 'Turkey Sausage', 'Home Style Potatoes'],
        Lunch: [
            'Butternut Squash Soup M',
            'Chicken Noodle Soup W',
            'Spinach Salad TN',
            'Corn Dogs W/M/E',
            'Mongolian Beef Bowl S/W',
            'Chickpea  Sandwich W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Sweet and Sour Chicken Stir Fry S/W (Hillside only)',
        ],
        Dinner: [
            'Butternut Squash Soup M',
            'Chicken Noodle Soup W',
            'Spinach Salad TN',
            'BBQ Chicken',
            'Baked Cheese Manicotti W/M/E (Hillside and Parkside only)',
            'Thai Curry Tofu S',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Sweet and Sour Chicken Stir Fry S/W (Hillside only)',
        ],
    },
    Thursday: {
        Breakfast: ['Waffle Sticks E/M/S/W', 'Sausage Patty', 'Country Style Potatoes'],
        Lunch: [
            'Cream of Asparagus Soup W/M/S',
            'Caldo de Res Soup',
            'Tuna Rotini Salad F/W/E',
            'Meatball Sub W/M/S/E (Hillside and Parkside only)',
            'Chicken Tacos',
            'Gardein Orange Chicken S/W',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Char Sui Pork Stir Fry S/W (Hillside only)',
        ],
        Dinner: [
            'Cream of Asparagus Soup W/M/S',
            'Caldo de Res Soup',
            'Tuna Rotini Salad F/W/E',
            'Banh Mi Chicken Sandwich W/S (Hillside and Parkside only)',
            'Beef Lomo Saltado S/W',
            'Pasta w/Vegan Meatballs S/W (Hillside and Parkside only)',
            "Chef's Choice",
            'Build Your Own Grain Bowl (Hillside only)',
            'Char Sui Pork Stir Fry S/W (Hillside only)',
        ],
    },
    Friday: {
        Breakfast: ['Turkey, Egg and Cheese on Croissant Sandwich E/M/S/W', 'Bacon', "Potatoes O'Brien"],
        Lunch: [
            'French Onion Soup',
            'Clam Chowder M/W/SF-C',
            'Chicken Caesar Salad w/Cilantro Dressing M/F',
            'Enchiladas M/S/W',
            'Battered Pollock F/W/M (Hillside and Parkside only)',
            'Spicy Korean Tofu Bowl S/W',
            "Chef's Choice",
            'Margherita Pizza M/W/S (Beachside and Hillside only)',
            'Build Your Own Grain Bowl (Hillside only)',
            'Kung Pao Tofu S/W (Hillside only)',
        ],
        Dinner: [
            'French Onion Soup',
            'Clam Chowder M/W/SF-C',
            'Chicken Caesar Salad w/Cilantro Dressing M/F',
            'Mac and Cheese Bar M/W/S (Hillside and Parkside only)',
            'Lemon Pepper Salmon F',
            'Ratatouille',
            "Chef's Choice",
            'Margherita Pizza M/W/S (Beachside and Hillside only)',
            'Build Your Own Grain Bowl (Hillside only)',
            'Kung Pao Tofu S/W (Hillside only)',
        ],
    },
    Saturday: {
        Brunch: [
            'Blueberry Pancakes M/W/S',
            'Lil Smokies',
            'Tater Tots',
            'Vegetable Noodle Soup W',
            'Chicken Tortilla Soup S/W',
            "Chef's Choice",
            'Build Your Own Crepe (Hillside only)',
        ],
        Dinner: [
            'Vegetable Noodle Soup W',
            'Chicken Tortilla Soup S/W',
            'Shrimp Fried Rice S/W/SF-C',
            'Peruvian Chicken',
            'Sweet Potato and Cauliflower Stew',
        ],
    },
    Sunday: {
        Brunch: [
            'French Toast Sticks E/M/W/S',
            'Sausage Links',
            'Hash Brown Patty',
            'Beef and Rice Soup',
            'Potato Leek  Soup M/S/W',
            "Chef's Choice",
        ],
        Dinner: ['Beef and Rice Soup', 'Potato Leek Soup M/S/W', 'Chicken Taquitos S/M/W', 'The Buddha Bowl'],
    },
};
